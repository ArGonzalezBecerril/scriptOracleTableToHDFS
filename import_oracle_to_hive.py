...
def run_script(sqoop_script):
    ...
    """
    # output_command = subprocess.check_output(sqoop_script, shell=True)
    output_command = subprocess.Popen([sqoop_script], shell=True, stdout=subprocess.PIPE).communicate()[0]
    return output_command


def build_init_scala(table, conn):
    # is_build_init = subprocess.check_output("cd sh && ./connection.sh " + table + " " + conn.schema_hive + " "
    #                                        + conn.path_hdfs_data_partition, shell=True)
...
def drop_table_hive_temp(conn, table):
    # is_deleted_table = subprocess.check_output("cd sh && ./drop_table_hive.sh "+conn.schema_hive + "." + table,
    #  shell=True)
...
for table in tables_oracle:
    ...
    build_init_scala(table, connection)
    #print(script_sqoop)
